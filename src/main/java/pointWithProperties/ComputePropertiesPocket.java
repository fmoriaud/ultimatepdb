...
public class ComputePropertiesPocket implements ComputePropertiesIfc {
	...
	// Constructor
	// -------------------------------------------------------------------
	public ComputePropertiesPocket(MyStructureIfc structure, AlgoParameters algoParameters, List<HBondDefinedWithAtoms> dehydrons, List<PointIfc> listOfLigandPoints){
		...
		if (distanceToAtomClosest <= fwhm){
			...
			if ((hDonnor != null && hDonnor > 0.1 ) || (hAcceptor != null && hAcceptor > 0.1 )){ // for performance needed only if donnor or acceptor not 0
				...
				if (orientationOK == false){
					...
					if (countOfHydrophobicAtomRelevantForHbond < algoParameters.getCUTOFF_COUNT_OF_HYDROPHOBIC_ATOM_SURRONDING_HBOND()){
						if (hDonnor != null){
							hDonnor = 0.0f;
						}
						if (hAcceptor != null){
