...
public class ExtendPairingRecursiveTask extends RecursiveTask<List<PairingAndNullSpaces>> {
    ...
    private ExtendPairing extendPairing;
    ...
    // Constructor
    // -------------------------------------------------------------------
    public ExtendPairingRecursiveTask(List<ResultsFromEvaluateCost> listResultSeed, int start, int end, int threshold, CollectionOfPointsWithPropertiesIfc shape1, CollectionOfPointsWithPropertiesIfc shape2, AlgoParameters algoParameters) {
        ...
        this.extendPairing = new ExtendPairing(shape1, shape2, algoParameters);
    }
    ...
    protected List<PairingAndNullSpaces> compute() {
        ...
        if (end - start < threshold) {
            ...
            for (int i = start; i <= end; i++) {
                ...
                ResultsFromEvaluateCost extendedSeed = extendSeed(result);
                listExtendedPairing.add(extendedSeed.getPairingAndNullSpaces());

            }

        } else {
            int midway = (end - start) / 2 + start;
            ExtendPairingRecursiveTask extendTask1 = new ExtendPairingRecursiveTask(listResultSeed, start, midway, threshold, shape1, shape2, algoParameters);
            extendTask1.fork();

            ExtendPairingRecursiveTask extendTask2 = new ExtendPairingRecursiveTask(listResultSeed, midway + 1, end, threshold, shape1, shape2, algoParameters);
    ...
    // Implementation
    // -------------------------------------------------------------------
    private ResultsFromEvaluateCost extendSeed(ResultsFromEvaluateCost resultSeed) {


        ResultsFromEvaluateCost extendedPairing = extendPairing.extendSeed(resultSeed);


        boolean isValid = PairingTools.validate(extendedPairing.getPairingAndNullSpaces());
        if (isValid == true) {
